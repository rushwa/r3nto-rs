...
pub fn ConversionPage() -> Element {
    ...
    let handle_initiate = move |_| {
        ...
        if uid.is_empty() || email.is_empty() {
            message.set(Some("Please enter BOTH the User ID (UUID) and Email".to_string()));
            is_error.set(true);
            return;
        }
        ...
        spawn(async move {
            match initiate_handshake(&t, &uid, &email).await {
                Ok(_) => {
    ...
    let handle_verify = move |_| {
        ...
        spawn(async move {
            match verify_handshake(&t, &uid, &email, &code).await {
                Ok(_) => {
    ...
    rsx! {
        div { class: "space-y-6 max-w-2xl",
                ...
                div { class: "flex gap-3",
                    ...
                    div {
                        ...
                        p { class: "text-gray-300 text-sm mt-1",
                            "Ask the client to log into their Rento account and share BOTH their User ID (UUID) and their Email address. "
            ...
            div { class: "bg-gray-800 rounded-lg border border-gray-700 p-6 space-y-4",
                div {
                    label { class: "block text-sm font-medium text-gray-400 mb-1", "Client's User ID (UUID)" }
                    input {
                        class: "w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-lg text-white font-mono text-sm focus:outline-none focus:border-blue-500",
                        r#type: "text",
                ...
                div {
                    label { class: "block text-sm font-medium text-gray-400 mb-1", "Client's Email" }
                    input {
